...
impl Driver {
    ...
    /// for the child.
    fn paste(&mut self, bytes: &[u8]) -> KtStatus {
        match self {
            Self::Detached(session) => status(session.paste(bytes)),
            // The run outlives the call on this side: it waits in a request
            // until the session's own thread comes for it.
            Self::Pty(session) => status(session.paste(bytes.to_vec())),
        }
    }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn kt_paste_is_safe(bytes: *const u8, len: usize) -> bool {
    guarded(false, || {
        let Ok(bytes) = (unsafe { entry::borrowed(bytes, len) }) else {
            return false;
        };
        knotty_core::paste_is_safe(bytes)
    })
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn kt_session_paste(
    session: *mut KtSession,
    bytes: *const u8,
    len: usize,
) -> KtStatus {
    entry::answer(|| {
        ...
        Ok(session.drive(|driver| driver.paste(bytes)))
    })
}
