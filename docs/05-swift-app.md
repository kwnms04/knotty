...
| 붙여넣기 경고 | 검사가 위험하다고 하면 경고 시트(미리보기 포함). 설정: 항상 / 멀티라인만(기본) / 끔 |

종료 경고("실행 중 프로세스")는 스냅샷의 자식 상태로 판정합니다.
**경고는 끌 수 있지만 정화는 끌 수 없습니다.** cf. [0007](adr/0007-input-security.md)

무엇이 위험한지는 **엔진이 정합니다** — `\n`이거나 내용에 `ESC[201~`가 있으면
위험입니다. 우리가 따로 세는 조건은 없습니다. 그래서 홑 `\r`만 있는 붙여넣기는
경고 없이 들어가며, 프롬프트에서는 그것도 실행됩니다. 엔진의 판정을 그대로 쓴다는
[0007](adr/0007-input-security.md)의 결정이 안고 가는 한계이고, 좁히려면 엔진 쪽에서
좁힙니다. 정화는 그것과 무관하게 언제나 걸립니다.
