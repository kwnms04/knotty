...
- **스톱갭은 절반만 회수됩니다.** 건전성 수정 둘은 `libghostty-vt` 안에 있으므로
  ...
  근거는 **패치에 남은 것이 무엇인가**입니다. 브랜치는 0.2.1 위의 커밋 하나로
  다시 잘랐고, diff는 `build.rs` +18/-1이 전부입니다. 이것은 영구히 들고 갈
  만한 종류입니다 — 상류가 흔들리면 빌드가 그 자리에서 깨지지, 건전성 수정처럼
  조용히 어긋나지 않습니다. shim 회귀는 `xcrun`을 `PATH`에 깔아 셸 전체에
  영향을 주므로 AGENTS.md가 이미 더 나쁜 쪽으로 기록해 두었습니다.

  다시 자르기 전에는 그렇지 않았습니다. 스톱갭 커밋이 `-sys`에도 `to_bytes`를
  하나 넣어 두었는데, 파사드는 자기 것을 쓰므로 부르지 않습니다 — 영구히 유지할
  포크에 **한 번도 부르지 않는 `unsafe`를 싣고 다니는 것**은 이 ADR이 막으려던
  바로 그 형태입니다. 그래서 `build.rs` 커밋만 남기고 `knotty-build-hooks`로
  다시 잘랐습니다. `build.rs`가 바이트 동일하고 `GHOSTTY_COMMIT`도 그대로이므로
  빌드되는 엔진은 비트 동일합니다.

  버린 절반은 상류가 이미 가져갔습니다 —
  [#74](https://github.com/Uzaaft/libghostty-rs/issues/74)는 closed입니다. 반면
  [#70](https://github.com/Uzaaft/libghostty-rs/issues/70)(`graphemes_buf`가
  용량을 넘기지 않음)은 **아직 open**이고, 파사드가 길이를 먼저 읽어 버퍼를 잡는
  것이 지금도 유효한 방어라는 뜻입니다.
