...
- **스톱갭은 절반만 회수됩니다.** 건전성 수정 둘은 `libghostty-vt` 안에 있으므로
  ...
  `-sys` 항목은 남습니다. 빌드 훅(`ZIG`, `ZIG_SYSROOT`)이 거기 있고, 파사드
  이후에도 `-sys`를 계속 쓰기 때문입니다. 이 훅은 건전성 수정과 성격이 다릅니다
  — additive하고 비파괴적이라 상류가 받아들일 여지가 있습니다. 다만 병합돼도
  crates.io 릴리스와 `=0.2.1` 핀 이동이 따라와야 하고, 핀 이동은 마일스톤
  사이에서만 하는 일입니다. **잠정적으로 `-sys` 패치는 유지하고, 파사드 착수
  시점에 상류 PR·영구 유지·shim 회귀 중에서 재검토합니다.**
